// ======= app.js (Физкультурный паспорт - Стандарт 2023 Беларусь) =======

(function insertCustomStyles() {
  const css = `
    .table-container { overflow-x: auto; border: 1px solid #e2e8f0; border-radius: 16px; background: white; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.02); padding-bottom: 8px; min-height: 200px; }
    table { width: 100%; border-collapse: separate; border-spacing: 0; }
    th, td { padding: 6px 8px; white-space: nowrap; border-bottom: 1px solid #f1f5f9; height: 54px; vertical-align: middle; }
    thead th { background: #f8fafc; font-weight: 600; color: #64748b; font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.05em; position: sticky; top: 0; z-index: 10; border-bottom: 1px solid #e2e8f0; text-align: center; }
    .sticky-col { position: sticky; left: 0; background: white; z-index: 20; border-right: 1px solid #e2e8f0; box-shadow: 4px 0 8px rgba(0,0,0,0.01); }
    ...
    .input-std { width: 100%; height: 34px; padding: 4px 4px 4px 8px; border: 1px solid #e2e8f0; border-radius: 8px; font-size: 14px; font-weight: 500; text-align: center; background: #fff; color: #334155; transition: all 0.2s; font-family: 'Inter', monospace; }
    .input-std:focus { border-color: #3b82f6; outline: none; box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.1); z-index: 5; position: relative; }
    .input-hint { position: absolute; bottom: -16px; left: 50%; transform: translateX(-50%); color: #94a3b8; font-size: 9px; pointer-events: none; font-weight: 500; white-space: nowrap; opacity: 0; transition: opacity 0.2s; }
    .input-group:hover .input-hint, .input-std:focus + .input-hint { opacity: 1; }
    .grade-badge { width: 28px; height: 28px; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 13px; font-weight: 800; flex-shrink: 0; background: #f1f5f9; color: #cbd5e1; transition: all 0.2s cubic-bezier(0.4, 0, 0.2, 1); }
    ...
    .student-card { background: white; border-radius: 16px; border: 1px solid #e2e8f0; margin-bottom: 12px; overflow: hidden; transition: transform 0.1s; }
    .student-card:active { transform: scale(0.99); }
    .card-header { padding: 16px; display: flex; justify-content: space-between; align-items: center; cursor: pointer; background: white; }
    ...
    .norm-item { background: white; padding: 12px; border-radius: 12px; border: 1px solid #e2e8f0; display: flex; flex-direction: column; gap: 8px; box-shadow: 0 1px 2px rgba(0,0,0,0.03); }
    .norm-controls { display: flex; gap: 8px; align-items: stretch; }
    .norm-grade { width: 42px; display: flex; align-items: center; justify-content: center; border-radius: 10px; font-weight: 800; font-size: 16px; background: #f1f5f9; color: #cbd5e1; flex-shrink: 0; }
    .floating { position: relative; margin-bottom: 16px; }
    .floating input, .floating select { width: 100%; padding: 20px 16px 8px; border: 1px solid #e2e8f0; border-radius: 14px; font-size: 16px; outline: none; transition: all 0.2s; appearance: none; background: white; height: 56px; }
    .floating label { position: absolute; left: 16px; top: 50%; transform: translateY(-50%); font-size: 15px; color: #94a3b8; pointer-events: none; transition: 0.2s; }
...
// ======= 1. Нормативы (Беларусь 2023 - Точное соответствие PDF) =======
// Примечание: endurance (runLong) настроен на 6-минутный бег (метры) как основной показатель.
// Силовые: Девочки 5-10 кл - низкая перекладина, 11 кл - отжимания. Мальчики - высокая перекладина.
...
function calculateGrade(normKey, value, sex, classNum, group) {
  ...
  // Если для данного пола/класса нет норматива (например, пресс у мальчиков или подтягивания у девочек 11 кл)
  ...
  // Логика расчета: сравниваем со значениями таблицы от 10 баллов до 1
  if (type === "speed") {
    // Для бега: чем меньше, тем лучше
    for (let i = 0; i < 10; i++) {
      // table[i] - это норматив на (10 - i) баллов
      if (table[i] !== null && val <= table[i]) {
        score = 10 - i;
        break;
      }
    }
    // Если результат хуже 1 балла (table[9]), то 1 (или 0, если совсем плохо, но по логике 1)
    if (score === 0 && table[9] !== null && val > table[9]) score = 1;
  } else {
    // Для силы/прыжков: чем больше, тем лучше
    for (let i = 0; i < 10; i++) {
      if (table[i] !== null && val >= table[i]) {
        score = 10 - i;
        break;
...
function renderStudents() {
  ...
  container.innerHTML = `
    ...
    <!-- Improved Modal with Validation -->
    <dialog id="studentModal" class="rounded-2xl shadow-2xl p-0 w-full max-w-[400px] backdrop:bg-black/40">
       <form method="dialog" class="bg-white flex flex-col h-full">
          ...
          <div class="p-6 bg-slate-50 flex justify-end rounded-b-2xl border-t border-slate-100">
  ...
  document.getElementById("mSave").onclick = () => {
    ...
    if (!nameVal || /[^а-яА-ЯёЁa-zA-Z\s\-]/.test(nameVal)) {
      ...
      err.innerText = "Только буквы, пробелы и дефис";
      nameField.parentNode.appendChild(err);
      isValid = false;
    }

    // Validate Year (>= 2000 and <= current year)
